...
namespace Safira::Middleware {
                              ...
                              ByteBuffer& scratch)
    {
        ...
        std::visit(Overloaded{
            ...
            [&](const FX::SendMessageHistoryTo& fx) {
                ...
                const std::size_t start = (history.size() > stateAfter.HistorySyncLimit)
    ...
    inline auto MakeServerEffectMiddleware(ServerEffectHandler handler) {
        ...
        {
            // Compute effects from the CURRENT state + this action
            auto result = ServerReduce(store.GetState(), action);

            // Let the reducer in the store also update state
            next(action);

            // Execute each effect (state in store is now updated)
