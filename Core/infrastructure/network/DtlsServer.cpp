...
namespace Safira {
    ...
    void Server::NetworkThreadFunc() {
        ...
        while (m_Running.load(std::memory_order_acquire)) {
            ...
            // Drive every client's state machine  (§3.4 typestate via visit)
            std::ranges::for_each(m_Clients | std::views::values,
                                  [this](ClientInfo& c) { DriveClient(c); });

            // Housekeeping — stale handshakes, send queue, kicks, dead clients
            ReapStaleHandshakes();
            DrainSendQueue();
            ProcessPendingKicks();

            // Reap dead clients (SSL reset = dead)
    ...
    // DispatchPacket
    //
    // Enforces MaxClients cap before creating a new session.
    ...
    // ─────────────────────────────────────────────────────────────────────────────
    // Typestate dispatch via std::visit
    // ─────────────────────────────────────────────────────────────────────────────
