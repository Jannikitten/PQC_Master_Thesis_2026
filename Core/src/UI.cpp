...
namespace Safira::UI {
	...
	ImRect RectExpanded(const ImRect& rect, const float x, const float y) {
		ImRect result = rect;
		result.Min.x -= x;
		result.Min.y -= y;
		result.Max.x += x;
	...
	void DrawButtonImage(const std::shared_ptr<Image>& imageNormal,
		...
		if (ImGui::IsItemActive())
			drawList->AddImage(imagePressed->GetDescriptorSet(),
				rectMin, rectMax, ImVec2(0, 0),
	...
	void DrawButtonImage(const std::shared_ptr<Image>& image,
		...
		const ImVec2 rectMin, const ImVec2 rectMax)
	{
		DrawButtonImage(image, image, image, tintNormal,
			tintHovered, tintPressed, rectMin, rectMax);
	};
	...
	void DrawButtonImage(const std::shared_ptr<Image>& imageNormal,
		...
		const ImU32 tintNormal, const ImU32 tintHovered, const ImU32 tintPressed) {
		DrawButtonImage(imageNormal, imageHovered, imagePressed, tintNormal,
			tintHovered, tintPressed, ImGui::GetItemRectMin(), ImGui::GetItemRectMax());
	};
	...
	void RenderWindowOuterBorders(ImGuiWindow* window) {
		...
		static constexpr ImGuiResizeBorderDef resize_border_def[4] = {
			{ ImVec2(+1, 0), ImVec2(0, 1),
				ImVec2(0, 0), IM_PI * 1.00f }, // Left
			{ ImVec2(-1, 0), ImVec2(1, 0),
				ImVec2(1, 1), IM_PI * 0.00f }, // Right
			{ ImVec2(0, +1), ImVec2(0, 0),
				ImVec2(1, 0), IM_PI * 1.50f }, // Up
			{ ImVec2(0, -1), ImVec2(1, 1),
				ImVec2(0, 1), IM_PI * 0.50f }  // Down
		};

		auto GetResizeBorderRect = [](const ImGuiWindow* window, const int border_n,
			const float perp_padding, const float thickness) {
			ImRect rect = window->Rect();
			if (thickness == 0.0f) {
				rect.Max.x -= 1;
				rect.Max.y -= 1;
			}
			if (border_n == ImGuiDir_Left) { return ImRect(rect.Min.x - thickness,
				rect.Min.y + perp_padding, rect.Min.x + thickness, rect.Max.y - perp_padding); }
			if (border_n == ImGuiDir_Right) { return ImRect(rect.Max.x - thickness,
				rect.Min.y + perp_padding, rect.Max.x + thickness, rect.Max.y - perp_padding); }
			if (border_n == ImGuiDir_Up) { return ImRect(rect.Min.x + perp_padding,
				rect.Min.y - thickness, rect.Max.x - perp_padding, rect.Min.y + thickness); }
			if (border_n == ImGuiDir_Down) { return ImRect(rect.Min.x + perp_padding,
				rect.Max.y - thickness, rect.Max.x - perp_padding, rect.Max.y + thickness); }
			IM_ASSERT(0);
			return ImRect();
		};
		...
		constexpr float border_size = 1.0f; // window->WindowBorderSize;
		if (border_size > 0.0f && !(window->Flags & ImGuiWindowFlags_NoBackground))
			window->DrawList->AddRect(window->Pos, { window->Pos.x + window->Size.x,
				window->Pos.y + window->Size.y }, ImGui::GetColorU32(ImGuiCol_Border),
				rounding, 0, border_size);

		if (const int border_held = window->ResizeBorderHeld; border_held != -1) {
			const auto&[InnerDir, SegmentN1, SegmentN2, OuterAngle] = resize_border_def[border_held];
			const ImRect border_r = GetResizeBorderRect(window, border_held, rounding, 0.0f);
			ImVec2 p1 = ImLerp(border_r.Min, border_r.Max, SegmentN1);
			const float offsetX = InnerDir.x * rounding;
			const float offsetY = InnerDir.y * rounding;
			p1.x += 0.5f + offsetX;
			p1.y += 0.5f + offsetY;

			ImVec2 p2 = ImLerp(border_r.Min, border_r.Max, SegmentN2);
			p2.x += 0.5f + offsetX;
			p2.y += 0.5f + offsetY;

			window->DrawList->PathArcTo(p1, rounding, OuterAngle - IM_PI * 0.25f, OuterAngle);
			window->DrawList->PathArcTo(p2, rounding, OuterAngle, OuterAngle + IM_PI * 0.25f);
	...
	bool UpdateWindowManualResize(ImGuiWindow* window, ImVec2& newSize, ImVec2& newPosition) {
		...
		auto CalcWindowAutoFitSize = [CalcWindowSizeAfterConstraint](ImGuiWindow* window, const ImVec2& size_contents) {
			...
			const ImVec2 size_pad{ window->WindowPadding.x * 2.0f, window->WindowPadding.y * 2.0f };
			const ImVec2 size_desired = { size_contents.x + size_pad.x + 0.0f, size_contents.y + size_pad.y + decoration_up_height };
			...
			const bool will_have_scrollbar_x = (size_auto_fit_after_constraint.x - size_pad.x - 0.0f < size_contents.x &&
			...
			const bool will_have_scrollbar_y = (size_auto_fit_after_constraint.y - size_pad.y - decoration_up_height < size_contents.y &&
		...
		const float resize_grip_draw_size = ImFloor(ImMax(g.FontSize * 1.10f, window->WindowRounding + 1.0f + g.FontSize * 0.2f));
		...
		auto CalcResizePosSizeFromAnyCorner = [CalcWindowSizeAfterConstraint](ImGuiWindow* window, const ImVec2& corner_target,
			...
			const ImVec2 pos_max = ImLerp({ window->Pos.x + window->Size.x, window->Pos.y + window->Size.y }, corner_target, corner_norm); // Expected window lower-right
			const ImVec2 size_expected = { pos_max.x - pos_min.x,  pos_max.y - pos_min.y };
		...
		auto GetResizeBorderRect = [](ImGuiWindow* window, int border_n, float perp_padding, float thickness) {
			...
			if (border_n == ImGuiDir_Left) { return ImRect(rect.Min.x - thickness, rect.Min.y + perp_padding,
				rect.Min.x + thickness, rect.Max.y - perp_padding); }
			if (border_n == ImGuiDir_Right) { return ImRect(rect.Max.x - thickness, rect.Min.y + perp_padding,
				rect.Max.x + thickness, rect.Max.y - perp_padding); }
			if (border_n == ImGuiDir_Up) { return ImRect(rect.Min.x + perp_padding, rect.Min.y - thickness,
				rect.Max.x - perp_padding, rect.Min.y + thickness); }
			if (border_n == ImGuiDir_Down) { return ImRect(rect.Min.x + perp_padding, rect.Max.y - thickness,
		...
		for (int resize_grip_n = 0; resize_grip_n < resize_grip_count; resize_grip_n++) {
			...
			if (held && g.IO.MouseDoubleClicked[0] && resize_grip_n == 0) {
				...
				const float x = g.IO.MousePos.x - g.ActiveIdClickOffset.x + ImLerp(def.InnerDir.x * grip_hover_outer_size,
					def.InnerDir.x * -grip_hover_inner_size, def.CornerPosN.x);
				const float y = g.IO.MousePos.y - g.ActiveIdClickOffset.y + ImLerp(def.InnerDir.y * grip_hover_outer_size,
	...
	bool BeginMenubar(const ImRect& barRectangle) {
		...
		const ImRect bar_rect = UI::RectOffset(barRectangle, 0.0f, padding.y);// window->MenuBarRect();
	...
	void EndMenubar() {
		...
		if (ImGui::NavMoveRequestButNoResultYet() && (g.NavMoveDir == ImGuiDir_Left ||
			g.NavMoveDir == ImGuiDir_Right) && (g.NavWindow->Flags & ImGuiWindowFlags_ChildMenu)) {
			...
			if (nav_earliest_child->ParentWindow == window && nav_earliest_child->DC.ParentLayoutType == ImGuiLayoutType_Horizontal && (g.NavMoveFlags & ImGuiNavMoveFlags_Forwarded) == 0)
			{
				...
				g.NavIdIsAlive = true; // Hide highlight for the current frame so we don't see the intermediary selection.
				g.NavIdIsAlive = g.NavMousePosDirty = true;
	...
	bool ButtonCentered(const char* label, const ImVec2& size) {
		...
		const float actualSize = ImGui::CalcTextSize(label).x + style.FramePadding.x * 2.0f;
		const float avail = ImGui::GetContentRegionAvail().x;

		if (const float off = (avail - actualSize) * 0.5f; off > 0.0f)
			ImGui::SetCursorPosX(ImGui::GetCursorPosX() + off);

		return ImGui::Button(label);
	}

	void DrawBorder(const ImRect rect, const float thickness, const float rounding, const float offsetX, const float offsetY) {
		auto min = rect.Min;
		min.x -= thickness;
		min.y -= thickness;
		min.x += offsetX;
