...
namespace Safira {
    namespace {
        ...
        public:
            ...
            key_exchange_groups() const override {
                return {Botan::TLS::Group_Params::ML_KEM_512};
            }
            ...
            allowed_signature_schemes() const override {
                return {Botan::TLS::Signature_Scheme(0x0904)};
            }
        };
            ...
            Botan::RandomNumberGenerator& rng)
        {
            ...
            if (std::filesystem::exists(keyPath) && std::filesystem::exists(certPath)) {
                Botan::DataSource_Stream keyStream(keyPath.string());
                std::shared_ptr<Botan::Private_Key> key = Botan::PKCS8::load_key(keyStream);
                auto cert = std::make_shared<Botan::X509_Certificate>(certPath.string());
                return KeyMaterial{ .Key = std::move(key), .Cert = std::move(cert) };
            }

            std::shared_ptr<Botan::Private_Key> key =
                Botan::create_private_key("ML-DSA", rng, "ML-DSA-4x4");
                ...
                const Botan::TLS::Policy&) override
            {
                if (chain.empty()) {
                    throw Botan::TLS::TLS_Exception(
                        ...
                        "empty certificate chain from peer");
                }

                const std::string fp = chain[0].fingerprint("SHA-256");
                ...
                switch (pin.Status) {
                    ...
                    case PinStatus::Mismatch:
                        throw Botan::TLS::TLS_Exception(
                            Botan::TLS::Alert::BadCertificate,
                            "peer fingerprint does not match stored pin");
                }
            }
    ...
    std::string BotanP2PCrypto::ProtocolDescription() {
        return "TLS 1.3 | ML-KEM-512 + ML-DSA-44";
    }
