...
try {
        ...
        std::filesystem::exists(certPath))
    {
        ...
        auto key = Botan::PKCS8::load_key(keySrc);

        Botan::DataSource_Stream certSrc(certPath.string());
        auto cert = std::make_shared<Botan::X509_Certificate>(certSrc);
    ...
    // fall through and regenerate credentials
}

// Generate fresh RSA identity
auto key = std::make_shared<Botan::RSA_PrivateKey>(rng, 3072);
