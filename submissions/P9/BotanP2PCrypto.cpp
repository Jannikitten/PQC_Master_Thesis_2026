...
[[nodiscard]] std::filesystem::path DataDir() {
    ...
    std::filesystem::path base = (home && *home) ? std::filesystem::path(home) / ".safira"
...
[[nodiscard]] PinResult VerifyOrTrustPeerFingerprint(const std::string &peerUsername,
    ...
    if (auto it = pins.find(peerUsername); it != pins.end()) {
        return {
            .Status = (it->second == fingerprint) ? PinStatus::Match : PinStatus::Mismatch,
...
class PQPolicy : public Botan::TLS::Default_Policy {
  public:
    ...
    key_exchange_groups_to_offer() const override {
        // Makes sort of sense I guess, just pick an option and return that
        //
        return {Botan::TLS::Group_Params::HYBRID_X25519_ML_KEM_768};
    }
    ...
    allowed_signature_schemes() const override {
        // No clue what this is about, what schemes are used for or what EDDSA_25519 is (except for
        // an Elliptic curve scheme)
        return {Botan::TLS::Signature_Scheme::EDDSA_25519};
    }
};
...
[[nodiscard]] KeyMaterial GenerateOrLoadCredentials(std::string_view identityName,
    ...
    try {
        Botan::DataSource_Stream keySource(keyPath.string());
    ...
    // Generate a fresh Ed25519 identity and self-sign a long-lived certificate.
    auto key = Botan::create_private_key("Ed25519", rng);
    if (!key)
        throw std::runtime_error("Botan::create_private_key returned null for Ed25519");

    Botan::X509_Cert_Options opts;
    opts.common_name = safeName;
    opts.not_before("20250101000000Z");
    opts.not_after ("20350101000000Z");

    // Ed25519 in Botan uses SHA-512 as the named hash for X.509 signing.
    auto cert = std::make_shared<Botan::X509_Certificate>(
        Botan::X509::create_self_signed_cert(opts, *key, "SHA-512", rng));
    ...
    {
        std::ofstream out(certPath, std::ios::trunc);
        out << cert->PEM_encode();
    }
...
class ServerCredentials : public Botan::Credentials_Manager {
  public:
    explicit ServerCredentials(const KeyMaterial &km) : m_Key(km.Key), m_Cert(km.Cert) {}

    std::vector<Botan::X509_Certificate>
    find_cert_chain(const std::vector<std::string> &key_types,
                    const std::vector<Botan::AlgorithmIdentifier> &,
                    const std::vector<Botan::X509_DN> &, const std::string &type,
                    const std::string &) override {
        // ── YOUR CODE HERE ──
        if (type != "tls-server" || !m_Cert)
            return {};

        // Only present our cert if the peer asked for a key type we actually have.
        const auto our_algo = m_Key ? m_Key->algo_name() : std::string{};
...
class TLSCallbacksAdapter : public Botan::TLS::Callbacks {
  public:
    TLSCallbacksAdapter(BotanP2PCrypto::Callbacks cbs, std::string peerUsername)
        : m_Cbs(std::move(cbs)), m_PeerUsername(std::move(peerUsername)) {}
    ...
    void tls_record_received(uint64_t /*seq_no*/, std::span<const uint8_t> data) override {
        ...
        m_Cbs.MessageReceived(m_PeerUsername, text, 0u);
    }
    ...
    void tls_alert(Botan::TLS::Alert alert) override {
        ...
        if (alert.type() == Botan::TLS::AlertType::CloseNotify) {
            ...
            if (m_Cbs.Closed)
                m_Cbs.Closed("peer sent close_notify");
            return;
        }
        if (alert.is_fatal() && m_Cbs.Closed)
            m_Cbs.Closed(std::string("fatal TLS alert: ") + alert.type_string());
    }
    ...
    void tls_verify_cert_chain(const std::vector<Botan::X509_Certificate> &chain,
        ...
        const auto fp = chain.front().fingerprint("SHA-256");
        const auto result = VerifyOrTrustPeerFingerprint(m_PeerUsername, fp);
        ...
        switch (result.Status) {
            ...
            case PinStatus::FirstUse:
                ...
                m_FirstUseFingerprint = result.Fingerprint;
                spdlog::warn("[P2P-Crypto] TOFU first-use for '{}' fingerprint {}",
                             m_PeerUsername, fp);
                return;
            case PinStatus::Mismatch:
                throw Botan::TLS::TLS_Exception(
                    Botan::TLS::Alert::BadCertificate,
                    std::format("peer '{}' fingerprint mismatch (expected {}, got {})",
                                m_PeerUsername, result.Fingerprint, fp));
        }
    }
...
void BotanP2PCrypto::Start(const Config &config, Callbacks callbacks) {
    ...
    impl->CallbacksAdapter =
        std::make_shared<TLSCallbacksAdapter>(std::move(callbacks), config.PeerUsername);
    ...
    if (config.IsResponder) {
        ...
        impl->Channel = std::make_unique<Botan::TLS::Server>(
            impl->CallbacksAdapter, impl->SessionMgr, impl->Creds, impl->Policy, impl->Rng, false);
    } else {
        impl->Creds = std::make_shared<ClientCredentials>();

        impl->Channel = std::make_unique<Botan::TLS::Client>(
            impl->CallbacksAdapter, impl->SessionMgr, impl->Creds, impl->Policy, impl->Rng,
...
void BotanP2PCrypto::Send(std::string_view message) {
    if (m_Impl && m_Impl->Channel)
        m_Impl->Channel->send(reinterpret_cast<const uint8_t *>(message.data()), message.size());
}
...
std::string BotanP2PCrypto::ProtocolDescription() {
    return "TLS 1.3 | X25519/ML-KEM-768 | Ed25519";
}
