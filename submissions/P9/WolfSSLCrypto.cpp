...
void WolfSSLCrypto::ConfigureKeyExchange(WOLFSSL_CTX *ctx) {
    // Rimelig intuitivt at skifte denne, og ProtocolDescription.
    // Svært at finde hvilken en man skulle skifte til.
    // https://www.wolfssl.com/documentation/manuals/wolfssl/appendix09.html hjalp meget.
    int groups[] = {WOLFSSL_SECP256R1MLKEM512};
...
std::string WolfSSLCrypto::ProtocolDescription() { return "DTLS 1.3 | SECP256R1MLKEM512"; }

// ─────────────────────────────────────────────────────────────────────────────
// GenerateSelfSignedCert — RSA-2048, SHA-256, 365-day validity
// ─────────────────────────────────────────────────────────────────────────────

std::expected<WolfSSLCredentials, WolfSSLCryptoError> WolfSSLCrypto::GenerateSelfSignedCert() {
    WC_RNG rng;
    if (wc_InitRng(&rng) != 0)
    ...
    int keySz = wc_RsaKeyToDer(&key, keyDer.data(), static_cast<word32>(keyDer.size()));
    if (keySz < 0) {
        wc_FreeRsaKey(&key);
        wc_FreeRng(&rng);
        return std::unexpected(WolfSSLCryptoError::CertificateGeneration);
    ...
    int certSz =
        wc_MakeSelfCert(&cert, certDer.data(), static_cast<word32>(certDer.size()), &key, &rng);
