    ...
    namespace {
        ...
        public:
            ...
            key_exchange_groups() const override {
                // ── YOUR CODE HERE ──
                return Botan::TLS::Default_Policy::key_exchange_groups();
            }

            [[nodiscard]] std::vector<Botan::TLS::Group_Params>
            key_exchange_groups_to_offer() const override {
                // ── YOUR CODE HERE ──
                return Botan::TLS::Default_Policy::key_exchange_groups_to_offer();
            }
            ...
            allowed_signature_schemes() const override {
                // ── YOUR CODE HERE ──
                return Botan::TLS::Default_Policy::allowed_signature_schemes();
            }
        };
            ...
            Botan::RandomNumberGenerator& rng)
        {
            ...
            // If that fails, generate fresh ones, persist them, and return.
               ...
               if (std::filesystem::exists(keyPath) && std::filesystem::exists(certPath)) {
                    std::string k, c;
                    { std::ifstream f(keyPath);  k.assign((std::istreambuf_iterator<char>(f)), {}); }
                    { std::ifstream f(certPath); c.assign((std::istreambuf_iterator<char>(f)), {}); }
                    if (!k.empty() && !c.empty()) return KeyMaterial{ .PrivateKeyPem = std::move(k), .CertPem = std::move(c) };
                }

                Botan::Ed25519_PrivateKey key(rng); // Possibly not quantum secure?
                Botan::X509_Cert_Options opts;
                opts.common_name = std::string(identityName);
                auto cert = Botan::X509::create_self_signed_cert(opts, key, rng, "Ed25519", 365);

                std::string keyPem  = Botan::PKCS8::PEM_encode(key);
                std::string certPem = cert.PEM_encode();

                { std::ofstream o(keyPath);  o << keyPem; }
                { std::ofstream o(certPath); o << certPem; }

                return KeyMaterial{ .PrivateKeyPem = std::move(keyPem), .CertPem = std::move(certPem) };
        }
        ...
        public:
            ...
            void tls_emit_data(std::span<const uint8_t> data) override {
                // ── YOUR CODE HERE ──
                if (m_Cbs.EmitData) m_Cbs.EmitData(data);
            }

            void tls_record_received(uint64_t /*seq_no*/,
                                     std::span<const uint8_t> data) override {
                // ── YOUR CODE HERE ──
                if (m_Cbs.MessageReceived) m_Cbs.MessageReceived(m_PeerUsername, std::string(data.data(), data.size()););
            }

            void tls_alert(Botan::TLS::Alert alert) override {
                // ── YOUR CODE HERE ──
                if (alert.type() == Botan::TLS::Alert::CLOSE_NOTIFY) m_CloseNotifyReceived = true;
            }

            void tls_session_activated() override {
                // ── YOUR CODE HERE ──
                m_Activated = true;
                if (m_Cbs.Activated) m_Cbs.Activated();
            }
