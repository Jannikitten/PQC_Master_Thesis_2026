...
namespace Safira {
    namespace {
        ...
        class PQPolicy final : public Botan::TLS::Default_Policy {
        public:
            ...
            Botan::RandomNumberGenerator& rng)
        {
            ...
            if (std::filesystem::exists(keyPath) && std::filesystem::exists(certPath)) {
                try {
                    ...
                    std::shared_ptr<Botan::Private_Key> key = Botan::PKCS8::load_key(keySrc);
                    auto cert = std::make_shared<Botan::X509_Certificate>(certPath.string());
                    return { .Key = std::move(key), .Cert = std::move(cert) };
                } catch (const std::exception& e) {
                    spdlog::warn("[P2P-Crypto] {}: failed to load credentials ({}) — regenerating",
                                 safeName, e.what());
                }
            }

            // Generate a fresh ECDSA P-256 keypair and a self-signed certificate.
            std::shared_ptr<Botan::Private_Key> key =
                Botan::create_private_key("ECDSA", rng, "secp256r1");
            if (!key)
                throw std::runtime_error("Failed to generate ECDSA key");

            Botan::X509_Cert_Options opts(safeName, 10u * 365u * 24u * 60u * 60u);
            opts.country      = "XX";
            opts.organization = "Safira";

            auto cert = std::make_shared<Botan::X509_Certificate>(
                Botan::X509::create_self_signed_cert(opts, *key, "SHA-256", rng));

            // Persist key and cert as PEM, then tighten file permissions.
            {
        ...
        public:
                ...
                const std::string&) override
            {
                if (type != "tls-server")
                    return {};
                const auto& algo = m_Key->algo_name();
                // C++20 ranges: check if our algo is in the accepted list.
                const bool accepted = key_types.empty()
                    || std::ranges::find(key_types, algo) != key_types.end();
                return accepted ? std::vector{ *m_Cert } : std::vector<Botan::X509_Certificate>{};
            }
        ...
        public:
            ...
            void tls_alert(Botan::TLS::Alert alert) override {
                if (alert.type() == Botan::TLS::Alert::CloseNotify) {
                    ...
                    spdlog::warn("[P2P-Crypto] TLS alert from {}: {}",
                                m_PeerUsername, alert.type_string());
                }
            }

            void tls_session_activated() override {
                m_Activated = true;
                spdlog::info("[P2P-Crypto] Handshake complete with {} — TLS 1.3 + X25519/ML-KEM-768",
                            m_PeerUsername);
                ...
                if (m_FirstUseTrusted && m_Cbs.MessageReceived) {
                    m_Cbs.MessageReceived(
                        "System",
                        std::format("First-use trust established for {} (fingerprint: {})",
                                    m_PeerUsername, m_FirstUseFingerprint),
                        0xFFCCAA66);
                }
            }
                ...
                const Botan::TLS::Policy&) override
            {
                if (chain.empty())
                    throw Botan::TLS::TLS_Exception(
                        ...
                        std::format("{} presented no certificate", m_PeerUsername));

                const auto& peer        = m_PeerUsername;
                const auto  fingerprint = chain.front().fingerprint("SHA-256");
                const auto  pin         = VerifyOrTrustPeerFingerprint(peer, fingerprint);

                switch (pin.Status) {
                    case PinStatus::Match:
                        spdlog::info("[P2P-Crypto] Fingerprint verified for {}", peer);
                        break;
                    case PinStatus::FirstUse:
                        m_FirstUseTrusted     = true;
                        m_FirstUseFingerprint = pin.Fingerprint;
                        spdlog::warn("[P2P-Crypto] TOFU: trusting new fingerprint for {}", peer);
                        break;
                    case PinStatus::Mismatch:
                        spdlog::error("[P2P-Crypto] Fingerprint MISMATCH for {} (pinned {}, got {})",
                                      peer, pin.Fingerprint, fingerprint);
                        throw Botan::TLS::TLS_Exception(
                            Botan::TLS::Alert::BadCertificate,
                            std::format("Fingerprint mismatch for {}", peer));
                }
            }
