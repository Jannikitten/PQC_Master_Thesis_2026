...
namespace Safira {
    ...
    void WolfSSLCrypto::ConfigureKeyExchange(WOLFSSL_CTX* ctx) {
        /* Use an ML-KEM hybrid named group (X25519/ML-KEM-512) so the
           TLS key-exchange is post-quantum secure (hybrid PQKEM + classical).
           Requires wolfSSL built with ML-KEM support. */
        int groups[] = { WOLFSSL_X25519MLKEM512 };
    ...
    std::string WolfSSLCrypto::ProtocolDescription() {
        return "DTLS 1.3 | X25519/ML-KEM-512";
    }
