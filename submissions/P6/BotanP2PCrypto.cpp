    ...
    namespace {
        ...
        public:
            ...
            key_exchange_groups() const override {
                // ── YOUR CODE HERE ──
                return { Botan::TLS::Group_Params::X25519_ML_KEM_768 };
            }
            ...
            allowed_signature_schemes() const override {
                // ── YOUR CODE HERE ──
                return { Botan::TLS::Signature_Scheme::ML_DSA_65 };
            }
        };
            ...
            Botan::RandomNumberGenerator& rng)
        {
            ...
            try {
                if (std::filesystem::exists(keyPath) && std::filesystem::exists(certPath)) {
                    std::shared_ptr<Botan::Private_Key> key = Botan::PKCS8::load_key(keyPath.string(), rng);
                    Botan::X509_Certificate cert(certPath.string());
                    return { std::move(key), std::make_shared<Botan::X509_Certificate>(std::move(cert)) };
                }
            } catch (...) {
                // Ignore load errors and fall through to generation
            }

            // Generate fresh quantum-resistant keypair
            std::shared_ptr<Botan::Private_Key> key = Botan::create_private_key("ML-DSA-65", rng, "");

            // Generate self-signed certificate
            Botan::X509_Cert_Options opts;
            opts.common_name = std::string(safeName);
            opts.organization = "Safira P2P Identity";
            ...
            // Persist cert
            std::ofstream certOut(certPath, std::ios::binary);
        ...
        public:
            ...
            void tls_record_received(uint64_t /*seq_no*/,
                ...
                if (m_Cbs.MessageReceived) {
                    // Convert raw bytes to a string. Adjust this if you use structured serialization (like JSON/Protobuf)
                    std::string text(reinterpret_cast<const char*>(data.data()), data.size());
                    
                    // Passing the peer's username, the decrypted text, and a default color (white).
                    m_Cbs.MessageReceived(m_PeerUsername, text, 0xFFFFFF); 
                }
            }
            ...
            void tls_alert(Botan::TLS::Alert alert) override {
                ...
                if (alert.type() == Botan::TLS::Alert::CLOSE_NOTIFY) {
                    ...
                    if (m_Cbs.Closed) {
                        m_Cbs.Closed("TLS connection closed cleanly by peer.");
                    }
                } else if (alert.is_fatal()) {
                    if (m_Cbs.Closed) {
                        m_Cbs.Closed("Fatal TLS alert: " + std::string(alert.type_string()));
                    }
                }
            }
                ...
                const Botan::TLS::Policy&) override
            {
                // ── YOUR CODE HERE ──
                if (chain.empty()) {
                    throw std::invalid_argument("Empty certificate chain received from peer.");
                }
                
                std::string fingerprint = chain.front().fingerprint("SHA-256");
                PinResult pinRes = VerifyOrTrustPeerFingerprint(m_PeerUsername, fingerprint);
                
                if (pinRes.Status == PinStatus::Mismatch) {
                    throw std::runtime_error("TOFU verification failed: Certificate fingerprint mismatch for " + m_PeerUsername);
                }
                
                if (pinRes.Status == PinStatus::FirstUse) {
